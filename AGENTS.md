...
`agentic_vm/main.py` contains the main CLI and VM lifecycle logic. `agentic_vm/spinner.py` contains terminal spinner support. `agentic-vm` is the thin shell wrapper that execs the Python entrypoint. Tests live in `tests/test_agentic_vm.py` and cover command behavior with mocked subprocess calls. Mkosi templates are stored under `mkosi/`, including the root filesystem repartition override at `mkosi/mkosi.repart/10-root.conf`.
